...
import { apiLogin, LoginResultType } from '../service';
import { modify_reset } from '@/api/api';
// const x = useSelector(x => x);

export const userInfo = (e: LoginResultType, phone: string) => {
  console.log([phone], e.user);
  // if (e.accessToken) {
  store.dispatch({
    type: 'SET_USER_INFO',
    payload: e,
  ...
  if (e.user.is_first_login) {
    // 首次登录-跳转重置密码
    window.location.href = '/system/recovery-pwd';
  } else {

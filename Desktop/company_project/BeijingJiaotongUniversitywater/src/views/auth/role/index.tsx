...
import { apiGetRoleList } from '../user/service';
import VerifyUtils from '@/utils/verifty';
import { apiRoleDelete } from '@/api/api';
...
const Orgstaff: FC = () => {
  ...
  const [pageList, setPage] = useState<{ pageNum: number; pageSize: number }>({
    pageNum: 1,
    pageSize: 10,
  });
  ...
  const apiRoleListFn = async () => {
    await apiGetRoleList({ ...pageList }).then((res: any) => {
      if (res.data.code === 200) {
        getDataSource(res.data.rows);
  ...
  const columns: ColumnsType<Column> = [
    {
      ...
      render: (value, record, index: number) => {
        return <span>{(pageList.pageNum - 1) * 10 + index + 1}</span>;
      },
    },
    {
      title: '角色名称',
          ...
          <TableEditTsx<Column>
            ...
            onHandelType={values => {
              if (values.btn_title === '编辑') {
                ...
                history.push(`/auth/role/editRole?id=${record.roleId}`);
              }
              if (values.btn_title === '查看') {
                changeShowCount(record);
                // setVisible(true);
                // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
                history.push(`/auth/role/roleDetail/${record.roleId}`);
              }
              if (values.btn_title === '删除') {
                console.log('7878', record);
                // eslint-disable-next-line
                apiRoleDelete({ roleId: record.roleId }).then((res: any) => {
                  if (res.data.code === 500)
                    VerifyUtils.Toast('info', res.data.msg);
                  if (res.data.code === 200) apiRoleListFn();
                });
              }
            }}
          />
  ...
  return (
    <div>
      <div className={classnames('Dashboard', style.searchBackground)}>
        <Row>
          {/* <Permission flag='9no8mblCZ0'> */}
          <Col span={24}>
            <Permission flag='system:role:add'>
              <div
                ...
                onClick={() => {
                  history.push('/auth/role/addRole');
                }}
              >
                <IconFont

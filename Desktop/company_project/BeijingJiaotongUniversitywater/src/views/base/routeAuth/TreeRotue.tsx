...
function TreeRoute(props: TreeRouteProps) {
  ...
  const fromIdBykey = (
    ...
    tree.forEach(item => {
      if (keys.includes(item.key ? `${item.key}` : '0')) {
  ...
  // 根据Key获取到所有的Id
  const getIdByKey = (keys: string[] = [], tree: TreeItemType[]) => {
  ...
  const onCheck = (
    ...
    if (setDefaultCheckedTreeIds) {
      const getNewIds: number[] = info?.checkedNodes
        ? info?.checkedNodes?.map((item: TreeItemType) =>
            item.id ? item.id : 0,
          )
        : [];
      const ids = [

...
function RouteAuth() {
  ...
  // tree的数据
  const [treeList, setTreeList] = useState<TreeItemType>({
    id: 0,
    menuId: 0,
    value: 0,
    menuType: 'M',
  ...
  // 当前选中的tree
  const [checkTreeDetail, setCheckTreeDetail] =
    useState<TreeItemType>(treeList);
  // 新增状态的回调
  ...
  const getTreeByKey = (key: string | number, tree: TreeItemType) => {
    let backTree = tree;
    if (tree.key === `${key}`) {
      backTree = tree;
    } else {
  ...
  const checkTreeFn = useCallback(
    (key: string | number) => {
      ...
      if (treeList) {
        const getTree: TreeItemType = getTreeByKey(key, treeList);
  ...
  const formatToTreeKey = (treeData: TreeItemType[], parentTreekey: string) => {
    ...
    toKeyTree.forEach((keyTree, _index) => {
      ...
      if (tree.children) {
        formatToTreeKey(tree.children, tree?.key ? `${tree?.key}` : '');
      }
    });
  };
  ...
  // 根据后台接口参数，结构化当前的目录结构
  const initTreeStructure = (treeData: TreeItemType[]) => {
    return formatToTree(treeData);
  };

  const initList = useCallback(async () => {
    const auth: TreeItemType = {
      id: 0,
      menuId: 0,
      menuType: 'M',
      value: 0,
    ...
    setLoading(true);
    try {
      const { data } = await apiGetAllPermissions();
      auth.children = initTreeStructure(data.data);
  ...
  const onRemove = () => {
    ...
    Modal.confirm({
      ...
      async onOk() {
        try {
          const { data } = await apiDeleteTreeById({
            permissionId: checkTreeDetail.permissionId
              ? checkTreeDetail.permissionId
              : 0,
          });
          if (data.code === 500) {
